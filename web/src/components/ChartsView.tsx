...
export default function ChartsView(props: { register: RegisterItem[], perangkatId: string, auth: AuthContext, repository: number[][], last_timestamp: string, timestamp_repo: string[], set_last_timestamp: (key: string) => void }) {
 
    const { isLoading, isSuccess, isError, data, error } = useQuery<Api<SensorData>, AxiosError>({
      queryFn: getSensorData,
      queryKey: [ `devices.${props.perangkatId}.data`, props.auth.token, props.perangkatId ],
      retry: false,
    ...
    if (data?.results.timestamp !== props.last_timestamp) {
      ...
      props.timestamp_repo.push(new Date(props.last_timestamp).toLocaleTimeString())
    ...
    if (isSuccess) return (
      <>
        ...
        <div className='grid grid-cols-3 gap-2 mt-6'>
          {props.repository.map((v,i) => <div key={`data.${i}`} className='rounded bg-gray-900 py-4 px-5 text-gray-200'>
            ...
            <div>
              ...
              }} data={{labels: props.timestamp_repo, datasets: [{
                label: 'data',
                data: v,
            ...
            <ul className='live-data max-h-48 overflow-auto'>
              {v.map((_, k) => (
                <li key={`${props.register[i][3]}-${k}`} className='flex justify-between'>
                  <p>{props.repository[i][props.repository[i].length - (k + 1)]} {props.register[i][2]}</p>
                  <p>{props.timestamp_repo[props.timestamp_repo.length - (k + 1)]}</p>
                </li>
              ))}
            </ul>
          </div>)}
