...
import { Register, RegisterItem, addRegister, getRegisterByPowermeter } from '../../../../api/register'
...
export const Route = createLazyFileRoute('/__auth/pengaturan/powermeter/$powermeterId/register/baru')({
...
function RegisterBaru() {
  const auth = useAuth()
    ...
    if (isSuccess) {
      console.log(dataBefore.results)
    ...
    const onFormSubmit = (evt: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        mutation.mutate({
          ...
          onSuccess: (res: AxiosResponse<Api<Register>, any>) => {
    ...
    return(
        <form className='register-table add-edit-register' onSubmit={onFormSubmit}>
            ...
            <Input
                type='text'
                name='nama'
                placeholder='ex: Tegangan A' />
            <Select 
                name='param'
                aria-label="Param Register"
            >
                <option value="222">ddd</option>
                <option value="222s">dddd</option>
            </Select>
            <div className='flex gap-4 p-2'>
                <button
                  type="submit"
                  className="text-sm text-white py-1 px-2 rounded-md w-full disabled:bg-gray-300 disabled:text-gray-500 border border-2 border-blue-900 transition hover:border-blue-500"
                >
